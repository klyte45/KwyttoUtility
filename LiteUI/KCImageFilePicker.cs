...
{
    public class KCImageFilePicker : GUIRootWindowBase
    {
        ...
        public static void PickAFile(string title, Action<string> onReturn)
        {
            var parent = UIView.GetAView();
            var container = new GameObject();
            container.transform.SetParent(parent.transform);
            var window = container.AddComponent<KCImageFilePicker>();
            window.Init(title ?? BasicIUserMod.Instance.GeneralName, new Rect(new Vector2((Screen.width / 2) - 400 * GUIWindow.ResolutionMultiplier, (Screen.height / 2) - 250 * GUIWindow.ResolutionMultiplier) / UIScaler.UIScale, new Vector2(800, 500) * GUIWindow.ResolutionMultiplier / UIScaler.UIScale), true, true, new Vector2(500, 500));
        ...
        private static string m_currentDirectory = BasicIUserMod.Instance.ModRootFolder;
        ...
        internal void RestartFilterCoroutine()
        {
            ...
            {
                BasicIUserMod.Instance.GetController().StopCoroutine(m_searchCoroutine);
            }
            m_searchCoroutine = BasicIUserMod.Instance.GetController().StartCoroutine(OnFilterParam(null));
        }
